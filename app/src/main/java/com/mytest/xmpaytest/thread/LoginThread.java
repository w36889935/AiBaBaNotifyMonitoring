...
public class LoginThread implements Runnable {
    ...
    public void run() {
        ...
        try {
            /**发送请求登陆对象**/
            HttpRespons httpRespons = requester.sendGet(ConfigurationProperties.LoginUrl,Requresparam);
            /**获得消息**/
            Message message=mHandler.obtainMessage();
            /**获得返回并且判断状态**/
            if(httpRespons.getContent().indexOf(IsSuccess)>0){
                /**将返回的数据转换为用户对象**/
                User user = JSON.parseObject(httpRespons.getContent().toString(), User.class);
                /**设置消息状态**/
                message.what=2;
                /**设置消息内容**/
                ...
                /**获得失败消息提示**/
                String messageBody = httpRespons.getContent().toString().replace("{\"errormsg\":\"","").replace("\"}","");
                /**设置消息状态**/
                message.what=3;
